...
class Widget_Pager extends Tiri_Widget{
    ...
    public static function show($total , $perPage){
        ...
        $ret = $_isHead = $_isTail = $_isMiddle = null;
        ...
        for($i = $start_page ;$i <= $end_page;$i++){
            if($i == $cur_page){
                $ret .='<div class="pager_current pager"><a href="javascript:void();">'.$i.'</a></div>';
        ...
        if($_isHead){
            ...
            $ret .='<div>...</div><div class="pager_tail pager"><a href="'.$url.'">'.$totalPageNum.'</a></div>';
        }
        if($_isTail){
            $url = Tiri_Request::getInstance()->getUrlWithQuery($_pagerParam , 1);
            $ret ='<div class="pager_tail pager"><a href="'.$url.'">1</a></div><div>...</div>' . $ret;
        }
        if($_isMiddle){
