...
class Tiri_Model{
    ...
    */
    static public function factory($modelName){
        /*
        if(!isset(self::$_loaded[$modelName])){
    ...
    * */
    public function save_to_db(){
        ...
        $pk = $this->_primary_key;
        /** 如果主键存在 */
        if(isset($this->_values[$pk]) && $this->_values[$pk] != NULL && is_numeric($this->_values[$pk])){
    ...
    */
    ...
    /**
    * @date 2013年10月24日17:30:49
    * @desc 常有检查是否存在的需求,之前常用loadById,消耗比较大
    * @param mixed $id
    */
    public function testPkExists($id){
        $pk = $this->_primary_key;
        $sql = "SELECT $pk FROM " . $this->_table . ' WHERE ' . $pk .' = '. $id;
        $db = Widget_Db::getInstance();
        $qry = $db->query($sql);
        $cnt = $db->num_rows($qry);
        if ($cnt == 0){
